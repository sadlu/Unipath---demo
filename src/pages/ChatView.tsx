...
export default function ChatView({ startChatUid, onBack }: ChatViewProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (lastMsgCount.current > prevCount) {
        showNotification('New message', `${activeUser?.display_name || activeChat} sent a message`)
      }
      const typing = await getTypingStatus(activeChat, userData.uid)
  ...
  useEffect(() => {
    if (activeChat) {
      markMessagesRead(userData.uid, activeChat)
    }
  }, [activeChat, messages, userData.uid])
  ...
  function handleTyping(value: string) {
    ...
    typingTimeout.current = setTimeout(() => {
      sendTypingIndicator(userData.uid, activeChat, false)
    }, 2000)
  }
  ...
  async function handleSend() {
    ...
    setSending(true)
    if (pendingImage) {
      await handleSendImage(pendingImage)
    ...
    return (
      <div className={`w-full ${isMobile ? 'px-3' : 'max-w-2xl mx-auto px-5'} pb-4 flex flex-col h-full`}>
            ...
            return (
              <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                ...
                >
                  ...
                  {isMe && (
                    <span className={`block text-right text-[10px] mt-0.5 ${m.read ? 'text-emerald-400' : 'text-slate-600'}`}>
                      {m.read ? 'Read' : 'Sent'}
                    </span>
                  )}
                </div>
        ...
        <div className="flex gap-2 pt-2 border-t border-white/5">
          ...
          />
          {otherTyping && (
            <div className="absolute -top-5 left-14 text-[11px] text-slate-500 italic animate-pulse">
              typing...
            </div>
          )}
          <input
