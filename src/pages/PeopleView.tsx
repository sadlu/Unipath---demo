...
export default function PeopleView({ onStartChat }: { onStartChat?: (uid: string) => void }) {
  ...
  return (
    <div className={`w-full ${isMobile ? '' : 'max-w-2xl mx-auto px-5'} pb-8 flex flex-col gap-4 mobile-native`}>
      ...
      <div className={isMobile ? 'px-4' : ''}>
        <div className="relative overflow-hidden">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-3 md:py-2.5 bg-[#1E1B2E] border border-[#2D2A3E] rounded-xl text-sm text-slate-200 placeholder-slate-500 focus:outline-none transition-all input-glow"
          />
          {query && (
      ...
      {selectedUser ? (
        <motion.div
          ...
          className="bg-[#1E1B2E] border border-[#2D2A3E] rounded-2xl p-5 flex flex-col gap-4"
        >
          ...
          {profileLoading ? (
            ...
            <div className="flex flex-col items-center gap-3">
              <div className="w-20 h-20 md:w-16 md:h-16 rounded-full bg-gradient-to-br from-[#7C5CFC] to-purple-600 flex items-center justify-center text-2xl md:text-xl font-black text-white overflow-hidden shadow-lg shadow-[#7C5CFC]/30">
              ...
              <div className="flex items-center gap-2 mt-2 w-full">
                ...
                <motion.button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 md:py-2.5 rounded-xl text-sm font-semibold transition-colors ${isFollowing ? 'bg-emerald-600/20 text-emerald-400 border border-emerald-600/30 hover:bg-emerald-600/30' : 'bg-[#0D0B18] text-slate-300 border border-[#2D2A3E] hover:border-[#7C5CFC]/50'}`}
                  whileTap={{ scale: 0.95 }}
                >
          ...
          <AnimatePresence>
            {results.map((u) => (
              <motion.button
                ...
                className={`w-full flex items-center gap-3 text-left ${isMobile ? 'native-list-row' : 'px-4 py-4 md:py-3 bg-[#1E1B2E] border border-[#2D2A3E] rounded-xl hover:bg-white/5 group'}`}
                whileHover={!isMobile ? { x: 4, borderColor: 'rgba(124,92,252,0.3)' } : undefined}
              >
