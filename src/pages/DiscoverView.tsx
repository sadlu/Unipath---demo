...
export default function DiscoverView() {
  ...
  const fetchCards = useCallback(async () => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('token') || msg.toLowerCase().includes('ratelimit')) {
        setError('Search is temporarily unavailable. Please try again later.')
      } else {
        setError(msg || 'Cannot reach the backend. Make sure the server is running and the Server URL is configured in Settings.')
      }
      setCards([])
    } finally {
      setLoading(false)
    ...
    return (
      <div className="w-full max-w-xl mx-auto px-4 flex flex-col items-center justify-center gap-4 mt-20">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={fetchCards}
          className="flex items-center gap-2 px-6 py-3 md:px-4 md:py-2 bg-[#7C5CFC] hover:bg-[#8D6CFF] rounded-xl text-white text-sm font-bold transition-all"
        >
  ...
  return (
    <div className={`w-full ${isMobile ? 'px-3' : 'max-w-5xl mx-auto px-4'} flex items-start justify-center gap-6 relative`}>
      <div className="w-full max-w-xl flex flex-col gap-5 relative" style={{ zIndex: 30 }}>
        <AnimatePresence mode="popLayout">
          <motion.div
            ...
            className="relative w-full bg-[#1E1B2E] border border-[#2D2A3E] rounded-3xl p-5 md:p-8 shadow-2xl cursor-grab active:cursor-grabbing origin-center overflow-hidden"
          >
            <motion.div
            ...
            >
              ...
              <motion.h2 variants={itemVariants} className="text-lg md:text-2xl font-extrabold text-white tracking-tight leading-snug text-balance mb-3">
                {activeItem.title}
              </motion.h2>
              ...
              <motion.div variants={itemVariants} className="mb-4">
                ...
                {descLong && (
                  <motion.button
                    ...
                    className="mt-1 flex items-center gap-1 text-xs font-semibold text-[#7C5CFC] hover:text-[#8D6CFF] transition-colors min-h-[28px]"
                  >
                    {expanded ? (
              ...
              <motion.div variants={itemVariants} className="flex flex-wrap gap-2 mb-4 border-t border-b border-white/5 py-4">
                <div className="flex items-center gap-1.5 bg-[#0D0B18] border border-[#2D2A3E]/50 px-3 py-1.5 md:py-1 rounded-xl text-xs text-slate-300">
                ...
                {activeItem.applyUrl && (
                  <motion.button
                    ...
                    className="flex items-center gap-1.5 bg-[#0D0B18] border border-[#2D2A3E]/50 px-3 py-1.5 md:py-1 rounded-xl text-xs text-sky-400 hover:bg-sky-500/10 hover:border-sky-500/30 transition-colors"
                  >
              ...
              <motion.div variants={itemVariants} className="grid grid-cols-2 gap-3 md:gap-4 mt-5 relative z-30">
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.97 }}
                  onClick={() => handleSwipe('left')}
                  className="flex items-center justify-center gap-2 py-4 md:py-3 bg-[#0D0B18] hover:bg-rose-950/20 border border-[#2D2A3E] hover:border-rose-900/30 rounded-xl text-slate-400 hover:text-rose-400 text-sm font-bold transition-all duration-150"
                >
                  <X className="w-4 h-4" />
                  <span>Skip</span>
                </motion.button>
                <motion.button
                  ...
                  className="flex items-center justify-center gap-2 py-4 md:py-3 bg-[#7C5CFC] hover:bg-[#8D6CFF] rounded-xl text-white text-sm font-bold transition-all duration-150 shadow-lg shadow-[#7C5CFC]/20"
                >
