...
export default function CVCoachView() {
  ...
  return (
    <div className={`w-full ${isMobile ? 'px-3' : 'max-w-3xl mx-auto px-5'} pb-4 flex flex-col h-full`}>
      <div className="flex items-center gap-3 py-3 border-b border-[#00F0FF]/10 mb-3 shrink-0">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#00F0FF] to-[#7C5CFC] flex items-center justify-center shadow-lg shadow-[#00F0FF]/20">
          <GraduationCap className="w-5 h-5 text-white" />
        </div>
        <div>
          <h2 className="text-lg font-extrabold text-white">CV Coach</h2>
          <p className="text-xs text-slate-500">AI-powered guide for your university journey</p>
        </div>
        {provider && (
          <div className="ml-auto flex items-center gap-1.5 px-2.5 py-1 bg-[#00F0FF]/10 border border-[#00F0FF]/20 rounded-full">
            <Sparkles className="w-3 h-3 text-[#00F0FF]" />
            <span className="text-[10px] font-semibold text-[#00F0FF]">{provider}</span>
          </div>
        )}
      </div>
          ...
          {QUICK_PROMPTS.map((prompt, i) => {
            ...
            return (
              <motion.button
                ...
                className="flex items-center gap-2 px-3 py-3 holo-glass rounded-xl hover:border-[#00F0FF]/30 text-left disabled:opacity-40"
      ...
      <div className="flex-1 overflow-y-auto min-h-0 flex flex-col gap-3 pr-1">
        <AnimatePresence initial={false}>
          {messages.map((msg, i) => (
            ...
            >
              {msg.role === 'user' ? (
                <div className="max-w-[85%] md:max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed bg-gradient-to-r from-[#7C5CFC] to-[#00F0FF] text-white rounded-br-md shadow-lg shadow-[#00F0FF]/20">
                  {msg.content}
                </div>
              ) : (
                <div className="max-w-[90%] md:max-w-[85%] px-4 py-3 rounded-2xl holo-glass text-slate-200 rounded-bl-md prose prose-invert prose-sm max-w-none prose-headings:text-white prose-headings:font-bold prose-headings:mt-4 prose-headings:mb-2 prose-p:text-slate-300 prose-p:leading-relaxed prose-p:my-1.5 prose-strong:text-[#00F0FF] prose-strong:font-bold prose-code:text-[#00F0FF] prose-code:bg-[#0A0817] prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-code:text-xs prose-ul:text-slate-300 prose-li:my-0.5 prose-a:text-sky-400 prose-a:no-underline hover:prose-a:underline prose-hr:border-[#00F0FF]/20 prose-blockquote:border-[#00F0FF]/50 prose-blockquote:text-slate-400">
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="holo-glass rounded-2xl rounded-bl-md px-4 py-3">
              <TypingDots />
            </div>
          </div>
        )}
      ...
      <div className="flex gap-2 pt-3 border-t border-[#00F0FF]/10 mt-3 shrink-0">
        <input
          type="text"
          value={input}
          ...
          disabled={loading}
          className="flex-1 px-4 py-3 md:py-2.5 holo-glass text-sm text-slate-200 placeholder-slate-500 focus:outline-none transition-all input-glow disabled:opacity-40"
        />
        <motion.button
          onClick={() => handleSend(input)}
          disabled={!input.trim() || loading}
          className="neon-btn px-5 py-3 md:py-2.5 disabled:opacity-40 rounded-xl"
          whileTap={{ scale: 0.9 }}
        >
