...
if DATABASE_URL:
    ...
    def upsert_user(uid: str, display_name: str, email: str = "", avatar_url: str = ""):
        with get_db() as conn:
            ...
            if cur.fetchone():
                _execute(conn, "UPDATE users SET display_name = ?, last_seen = NOW() WHERE uid = ?", (display_name, uid))
            else:
                _execute(conn, "INSERT INTO users (uid, display_name, email, avatar_url, last_seen) VALUES (?, ?, ?, ?, NOW())", (uid, display_name, email, avatar_url))
    ...
    def create_verification_code(email: str) -> str:
        ...
        with get_db() as conn:
            _execute(conn, "INSERT INTO email_verifications (email, code, created_at) VALUES (?, ?, NOW()) ON CONFLICT (email) DO UPDATE SET code = ?, created_at = NOW()", (email, code, code))
        return code

    def verify_code(email: str, code: str) -> bool:
        with get_db() as conn:
            cur = _execute(conn, "SELECT code FROM email_verifications WHERE email = ?", (email,))
            row = cur.fetchone()
    ...
    def create_auth_token(uid: str) -> str:
        ...
        with get_db() as conn:
            _execute(conn, "INSERT INTO auth_tokens (token, uid) VALUES (?, ?)", (token, uid))
        return token

    def get_uid_by_token(token: str) -> str | None:
        with get_db() as conn:
            cur = _execute(conn, "SELECT uid FROM auth_tokens WHERE token = ?", (token,))
            row = cur.fetchone()
    ...
    def create_account(uid: str, display_name: str, password: str, subjects: list[str] | None = None) -> dict:
        ...
        with get_db() as conn:
            _execute(conn, "INSERT INTO users (uid, display_name, password_hash, subjects, streak_days, last_seen) VALUES (?, ?, ?, ?, 0, NOW())", (uid, display_name, pwh, subj_str))
        return get_user(uid)
    ...
    def authenticate(uid: str, password: str) -> dict | None:
        ...
        if not stored or not verify_password(password, stored):
            return None
        with get_db() as conn:
            _execute(conn, "UPDATE users SET last_seen = NOW() WHERE uid = ?", (uid,))
        return user
    ...
    def upsert_user(uid: str, display_name: str, email: str = "", avatar_url: str = ""):
        with get_db() as conn:
            ...
            if cur.fetchone():
                conn.execute("UPDATE users SET display_name = ?, last_seen = datetime('now') WHERE uid = ?", (display_name, uid))
            else:
                conn.execute("INSERT INTO users (uid, display_name, email, avatar_url, last_seen) VALUES (?, ?, ?, ?, datetime('now'))", (uid, display_name, email, avatar_url))
    ...
    def search_users(query: str, exclude_uid: str = "", limit: int = 20) -> list[dict]:
        with get_db() as conn:
            ...
            if exclude_uid:
                exclude_clause = "AND uid != ?"
                params.append(exclude_uid)
            cur = conn.execute(f"SELECT * FROM users WHERE (display_name LIKE ? OR email LIKE ?) {exclude_clause} ORDER BY last_seen DESC LIMIT ?", (*params, limit))
            return [dict(r) for r in cur.fetchall()]

    def create_verification_code(email: str) -> str:
        code = "".join(random.choices(string.digits, k=6))
        with get_db() as conn:
            conn.execute("INSERT OR REPLACE INTO email_verifications (email, code, created_at) VALUES (?, ?, datetime('now'))", (email, code))
        return code

    def verify_code(email: str, code: str) -> bool:
        with get_db() as conn:
            cur = conn.execute("SELECT code FROM email_verifications WHERE email = ?", (email,))
            row = cur.fetchone()
            if row and row["code"] == code:
    ...
    def send_message(from_uid: str, to_uid: str, content: str, image_url: str = "") -> dict:
        with get_db() as conn:
            cur = conn.execute("INSERT INTO messages (from_uid, to_uid, content, image_url) VALUES (?, ?, ?, ?)", (from_uid, to_uid, content, image_url))
    ...
    def get_conversations(uid: str) -> list[dict]:
        with get_db() as conn:
            ...
            for other_uid in other_uids:
                ...
                last_row = cur.fetchone()
                cur = conn.execute("SELECT COUNT(*) AS cnt FROM messages WHERE to_uid = ? AND from_uid = ? AND read = 0", (uid, other_uid))
    ...
    def get_messages(uid_a: str, uid_b: str, limit: int = 50) -> list[dict]:
        with get_db() as conn:
            ...
            rows = cur.fetchall()
            conn.execute("UPDATE messages SET read = 1 WHERE to_uid = ? AND from_uid = ? AND read = 0", (uid_a, uid_b))
    ...
    def follow_user(follower_uid: str, following_uid: str) -> bool:
        ...
        with get_db() as conn:
            try:
                conn.execute("INSERT OR IGNORE INTO follows (follower_uid, following_uid) VALUES (?, ?)", (follower_uid, following_uid))
    ...
    def unfollow_user(follower_uid: str, following_uid: str) -> bool:
        with get_db() as conn:
            conn.execute("DELETE FROM follows WHERE follower_uid = ? AND following_uid = ?", (follower_uid, following_uid))
            return conn.total_changes > 0

    def get_following(uid: str) -> list[str]:
        with get_db() as conn:
            cur = conn.execute("SELECT following_uid FROM follows WHERE follower_uid = ? ORDER BY created_at DESC", (uid,))
            return [r["following_uid"] for r in cur.fetchall()]

    def get_followers(uid: str) -> list[str]:
        with get_db() as conn:
            cur = conn.execute("SELECT follower_uid FROM follows WHERE following_uid = ? ORDER BY created_at DESC", (uid,))
            return [r["follower_uid"] for r in cur.fetchall()]

    def is_following(follower_uid: str, following_uid: str) -> bool:
        with get_db() as conn:
            cur = conn.execute("SELECT 1 FROM follows WHERE follower_uid = ? AND following_uid = ?", (follower_uid, following_uid))
            return cur.fetchone() is not None
    ...
    def create_account(uid: str, display_name: str, password: str, subjects: list[str] | None = None) -> dict:
        ...
        subj_str = ",".join(subjects) if subjects else ""
        with get_db() as conn:
            conn.execute("INSERT INTO users (uid, display_name, password_hash, subjects, streak_days, last_seen) VALUES (?, ?, ?, ?, 0, datetime('now'))", (uid, display_name, pwh, subj_str))
        return get_user(uid)
